...
class ResultsCollector(core.Callback):
    ...
    def on_validation_end(self, loss: float, logs: core.Interaction, epoch: int):
        ...
        # For 'BeS' description of target node:
        test_metrics["target_node"] = [item for sublist in logs.aux_input for item in sublist]

        # For sequence description (non-uniform data):
        # test_metrics["sequence"] = [seq.tolist() for seq in logs.sender_input]

        # For int description of target node:
        # test_metrics["target_node"] = [item for sublist in logs.aux_input for item in sublist.tolist()]

        # For message entropy (not implemented yet):
        ...
        self._print_to_console({k: v for k, v in test_metrics.items() if k not in ['messages', 'sequence', 'target_node']})
