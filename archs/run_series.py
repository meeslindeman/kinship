...
def run_experiment(opts: Options, target_folder: str, save: bool = True):
    ...
    if opts.root == 'data/':
        dataset = FamilyGraphDataset(root=opts.root)
    elif opts.root == 'data_uniform/':
        dataset = UniformFamilyGraphDataset(root=opts.root)
    else:
        raise ValueError(f"Dataset not found: {opts.root}")
...
def run_series_experiments(opts: List[Options], target_folder: str):
    """
